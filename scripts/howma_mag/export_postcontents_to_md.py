...
def main() -> int:
    ...
    parser.add_argument("--log-file", default="", help="Run log path. Defaults to _export_postcontents.log in outdir")
    args = parser.parse_args()

    input_path = Path(args.input).resolve()
    outdir = Path(args.outdir).resolve()
    outdir.mkdir(parents=True, exist_ok=True)
    log_path = Path(args.log_file).resolve() if args.log_file else outdir / "_export_postcontents.log"
    log_path.write_text("", encoding="utf-8")
    log_line(log_path, f"Started: {dt.datetime.now().isoformat(timespec='seconds')}")
    log_line(log_path, f"Input CSV: {input_path}")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        log_line(log_path, f"[{index}/{len(rows)}] {out_path.name} <- {url}")
    ...
    if errors:
        ...
        log_line(log_path, f"Wrote errors: {error_path}")

    log_line(log_path, f"Wrote markdown files to: {outdir}")
    log_line(log_path, f"Errors: {len(errors)}")
